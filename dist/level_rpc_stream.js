...
var msgpack_1 = require("mux-demux/msgpack");
var duplexify_1 = require("duplexify");
...
function createLevelRPCStream(level) {
    ...
    var levelInStream = through2_1["default"].obj(function (_a, enc, cb) {
        var id = _a.id, op = _a.op, args = _a.args;
        ...
        function handleOperationStream(id, stream) {
            ...
            stream.on('error', function (err) { return substream.error(err.message); });
        }
    });
    if (level.isOpen()) {
        ...
        inStream.pause();
        level.once('open', function () { return inStream.pipe(levelInStream); });
    }
